function V = nnlsHALSupdt_fixedsupp(M,U,V,maxiter,locus)
...
%
% This is a quick adaptation for a known support of V (Octobre 2018).
%
...
%   locus : known positions of zeros in V
%
...
while eps >= (delta)^2*eps0 && cnt <= maxiter %Maximum number of iterations
    nodelta = 0; if cnt == 1, eit3 = cputime; end
        for k = 1 : r
            ...
            for pos2=1:size(locus,1)
                deltaV(locus(pos2,:)==k) = 0; 
            end
            V(k,:) = V(k,:) + deltaV;
