function [A_aux,B_aux,C_aux,K_aux,err_anls_vec]=     Dmult_mpnals(Y,iter,A_0,B_0,C_0,D,contents,opts)
%-------------------------------------------------------------------------%
% [A_aux,B_aux,C_aux,K_aux,err_anls_vec]=     Dmult_mpnals(Y,iter,A_0,B_0,C_0)
%
...
% - D           : known dictionaries for factor B. Use cell
% - contents    : row vector containing number of atoms to be selected in
% each dictionary in cell D.
% - opts        : options. 
...
if strcmp(opts{1},'NN')    
    ...
    % OMP step
    [alloc,indexes,B_aux]    =     MatchP_multiD(B_aux,D,contents);
    K_aux             =     [alloc;indexes];
