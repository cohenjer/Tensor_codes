function  [A_aux,B_aux,C_aux,K_aux,P,Y_est,err_P2MP] = ...
          D_P2mpnals(Y,iter,A_0,B_0,C_0,D)
%-------------------------------------------------------------------------%
% [A_aux,B_aux,C_aux,err_als_vec] = ...
%           D_P2mpnals(Y,iter,A_0,S_0,C_0,D)
%
...
%--------------------------NALS one iteration-----------------------------%
    ...
    % A factors update
    A_aux             =     nnlsHALSupdt(Y_mode1',kr(C_aux,B_aux),A_aux',10)';
    
    % Normalization
    A_aux       =     A_aux.*repmat(1./sqrt(sum(A_aux.^2)),dim(1),1);
    
    % B factors update
    B_aux             =     nnlsHALSupdt(Y_mode2',kr(C_aux,A_aux),B_aux',10)';
    ...
    % C factors update
    C_aux             =     nnlsHALSupdt(Y_mode3',kr(B_aux,A_aux),C_aux',10)';
