function [A_aux,B_aux,C_aux,K_aux,B_flex,err_anls_vec]=     D_smpnals(Y,iter,A_0,B_0,C_0,D,inc,opts)
%-------------------------------------------------------------------------%
% [A_aux,B_aux,C_aux,err_als_vec]=     D_mpnals_flex(Y,iter,A_0,B_0,C_0)
%
...
% parameter lambda when estimating B. K is used for estimating A and C.
%
% Inputs:
% 
% - Y           : data block;
...
if strcmp(opts{1},'NN')
    if strcmp(opts{2},'true')
        A_aux             =     nnlsHALSupdt(Y_mode{1}',kr(C_aux,D(:,K_aux)),A_aux',inner)';
    else
        A_aux             =     nnlsHALSupdt(Y_mode{1}',kr(C_aux,D(:,K_aux)))';
    end
else
...
if strcmp(opts{1},'NN')
    ...
    %B_aux             =     col_norm(B_aux);
    % MP step
    if norm(B_aux-D(:,K_aux))/norm(B_aux) > 0.01
        delta = delta*inc; 
...
if strcmp(opts{1},'NN')
    if strcmp(opts{2},'true')
    C_aux             =     nnlsHALSupdt(Y_mode{3}',kr(D(:,K_aux),A_aux),C_aux',inner)';
    else
    C_aux             =     nnlsHALSupdt(Y_mode{3}',kr(D(:,K_aux),A_aux))';
    end
else
    ...
    temp_BA           =     (D(:,K_aux)'*D(:,K_aux)).*(A_aux'*A_aux);
    Y_BA              =     Y_mode{3}*kr(D(:,K_aux),A_aux);
